...
- **課程關聯**：每位學生都屬於特定課程
- 學號唯一性驗證
- **依學號排序**：學生列表（含成績登記個人模式）一律依學號升冪排列（`GET /api/students` 的 `orderBy: { studentId: 'asc' }`）。學號為等長純數字時，字典序即等同數值序（例：`111001 → 111002 → 112001`）
...
- **📋 複製填分腳本 / 🧩 Chrome 插件一鍵填成績（校務系統 portalx 用）**：
  - 適用學校校務系統不支援 Excel 匯入、只能靠 DevTools Console 手動貼分數的情境
  - 一鍵把 `{ 學號: 分數 }` 對應內嵌進腳本複製到剪貼簿；插件則以 `chrome.scripting` 直接注入成績登錄頁
  - **按學號比對欄位**（非按位置順序），portalx 排序/分頁不同也不會錯位
  - 找不到對應學號的列**自動跳過**，不會誤填非本班學生；不在本班名單的學生永遠不會被填
  - 可勾選「未登記成績也填 0 分」（預設不勾，避免覆蓋校務系統上既有分數）
  - 在校務系統成績登錄頁按 F12 → Console 貼上 → Enter 即填入；執行後 alert 顯示防呆統計
  - **穩健比對（避免整批「找不到欄位」）**：每列蒐集所有符合學號正則的候選逐一比對（不會被 WebForms 控制項 id 內的數字搶走）；比對前兩側正規化（全形→半形、去空白、大寫，**僅供比對、不改 DB 值**）；同時掃描同源 iframe/frame（插件版用 `allFrames` 注入、注入前輪詢等表格 render）。
  - **失敗時完整診斷**（符合「所有錯誤完整顯示在前端」）：顯示掃描列數 / 有輸入框列數 / frame 數，以及「頁面實際抓到的學號」vs「系統要填的學號」，一眼判斷是「掃不到欄位」還是「格式對不上」。
  - 腳本最上方的 `CONFIG` 可調整 selector 與學號正則以適配實際頁面結構
  - 填值演算法集中於 `src/lib/portalSync.ts`；**鏡像於三處，改一處要三處同步**：`FILL_FUNCTION_SOURCE`（Console 腳本）、`extension/background.js` 的 `pageFill`（插件一鍵）、`worker/portalUpload.mjs`（已停用，保留備查）
