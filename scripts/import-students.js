...
// 主程式
async function main() {
  const excelFilePath = '/Users/eric/Downloads/1141_IC335_A.xls';
  
  console.log('開始讀取Excel檔案:', excelFilePath);
  const rawData = readExcelFile(excelFilePath);
  
  if (!rawData) {
    console.error('無法讀取Excel檔案');
    return;
  }
  ...
  if (students.length === 0) {
    console.error('沒有找到有效的學生資料');
    return;
  }
  
  // 將解析結果保存為JSON檔案
  const outputPath = path.join(__dirname, '../temp-students-a.json');
  fs.writeFileSync(outputPath, JSON.stringify(students, null, 2), 'utf8');
  console.log('學生資料已保存到:', outputPath);
...
module.exports = { readExcelFile, parseStudentData };